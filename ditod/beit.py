...
class BEiT(nn.Module):
    ...
    def __init__(self,
        ...
        if patch_size == 16:
            ...
            self.fpn3 = nn.Identity() # 操作不变

            self.fpn4 = nn.MaxPool2d(kernel_size=2, stride=2) # 两倍下采样
    ...
    def forward_features(self, x):
        ...
        for i in range(len(features)):
            # print(f'-----features[{i}]: ', features[i].size()) # torch.Size([1, 768, 46, 66])
            features[i] = ops[i](features[i])

        feat_out = {}

        for name, value in zip(self.out_features, features):
            # 得到每个特征层的输出 OUT_FEATURES: ["layer3", "layer5", "layer7", "layer11"]
            feat_out[name] = value
