...
class VIT_Backbone(Backbone):
    ...
    def __init__(self, name, out_features, drop_path, img_size, pos_type, model_kwargs):
        ...
        if 'base' in name: # dit_base_patch16
        ...
        if name == 'beit_base_patch16':
            ...
            model_func = dit_base_patch16 # 从beit调用dit_base_patch16网络
            self._out_feature_channels = {"layer3": 768, "layer5": 768, "layer7": 768, "layer11": 768}
...
def build_VIT_backbone(cfg):
    ...
    """
    # fmt: off
    name = cfg.MODEL.VIT.NAME # dit_base_patch16
...
def build_vit_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    """
    # backbone调用vit
    bottom_up = build_VIT_backbone(cfg) # 得到特征层输出
    in_features = cfg.MODEL.FPN.IN_FEATURES #  IN_FEATURES: ["layer3", "layer5", "layer7", "layer11"]
    out_channels = cfg.MODEL.FPN.OUT_CHANNELS   # OUT_CHANNELS: 256
    backbone = AC_FPN(
        bottom_up=bottom_up,
